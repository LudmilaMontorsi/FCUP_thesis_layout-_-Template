...
The main question of the study, the primary outcome, is which image better shows the anatomy under the metal artifact, taken from the corresponding question described above. Since each doctor answers this question for every case, and every case is judged by every doctor, a statistical model was used that takes both of these repetitions into account at the same time, instead of simply counting how many times each answer was given. The other three per-case questions, together with the final usefulness question, are treated as secondary outcomes and are summarized more simply, as the frequency of each answer with a 95\% confidence interval.

Because two cases are shown a second time to each doctor later in the same session, without saying so, it is possible to check how consistent each doctor is with themselves. For this consistency check, the intraclass correlation coefficient ICC(2,1) was used for answers on a scale, and Fleiss's $\kappa$ was used for the simple output-versus-input preference. Both are standard ways of answering the question ``if the same case is shown twice to the same doctor, how often do they answer the same way'', adapted here to work with only two repeats per doctor. Given the small number of doctors expected to take part, all of these analyses were planned to be simple and robust to a small sample, favoring frequencies and confidence intervals over more elaborate models wherever possible.
