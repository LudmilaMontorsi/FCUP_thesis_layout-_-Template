...
Among the six checkpoints previously frozen by validation\acrshort{ssim}, the Single Model with seed 2024 had the highest mean patient-level Artifact-ROI $\Delta\acrshort{ssim}$ ($0.3279 \pm 0.0955$). Its mean Clean-ROI $\Delta\acrshort{ssim}$ was $0.0581 \pm 0.0422$, substantially smaller than the improvement observed in the Artifact Zone. This meets the preservation check that was planned in advance. But since no specific numeric limit was set for how small the Clean-Zone change should be, it should not be read as formal proof of anatomical safety. The Single Model, seed 2024, was therefore selected for external evaluation, after which its architecture, seed, weights, and hyperparameters remained unchanged. The other\acrshort{roi} metrics reported above are complementary and did not change this decision. The global metrics, reported next for completeness, are likewise complementary. 
...
\begin{table}[htbp]
    ...
    \captionsetup{skip=2pt}
    \caption[Patient-level global $\Delta$SSIM]{$\Delta\acrshort{ssim}$ global aggregated per patient in the test subset ($n=41$). CI: 95\% confidence interval.}
...
\begin{table}[htbp]
    ...
    \captionsetup{skip=2pt}
    \caption[Patient-level global $\Delta$PSNR]{$\Delta\acrshort{psnr}$ global aggregated per patient in the test subset ($n=41$). CI: 95\% confidence interval.}
...
\begin{table}[htbp]
    ...
    \captionsetup{skip=2pt}
    \caption[Patient-level global $\Delta$MAE]{$\Delta\acrshort{mae}$ global aggregated per patient in the test subset ($n=41$). Values $>0$ indicate improvement. CI: 95\% confidence interval.}
...
\captionsetup{skip=2pt}
\captionof{table}[Clean-ROI SSIM preservation magnitude]{Magnitude of SSIM input-to-output change in the Clean ROI.}
...
\par\smallskip\footnotesize Patient-level mean absolute per-slice input-to-output change ($n=41$). Lower values are closer to zero. No equivalence margin or hypothesis test against zero was prespecified.
\captionsetup{skip=2pt}
\caption[Clean-ROI PSNR preservation magnitude]{Magnitude of PSNR input-to-output change in the Clean ROI.}
...
\par\smallskip\footnotesize Patient-level mean absolute per-slice input-to-output change ($n=41$). Lower values are closer to zero. No equivalence margin or hypothesis test against zero was prespecified.
\captionsetup{skip=2pt}
\caption[Clean-ROI MAE preservation magnitude]{Magnitude of MAE input-to-output change in the Clean ROI.}
...
\captionsetup{skip=2pt}
\caption[Paired Clean-ROI preservation advantage]{Relative preservation advantage of the Dual Model in the Clean ROI.}
...
This distribution check was done separately for each model before applying the same rule to both. For $\Delta$SSIM, the Shapiro--Wilk test showed the Single Model's values were not evenly distributed ($W=0{.}9158$, $p=0{.}0050$, skewness $=1{.}087$). The same was true for the Dual Model ($W=0{.}9162$, $p=0{.}0052$, skewness $=1{.}056$), supporting the use of the sign test for both architectures. For $\Delta$MAE, the same pattern was found in the Single Model ($W=0{.}7607$, $p=8{.}95\times10^{-7}$, skewness $=2{.}281$) and in the Dual Model ($W=0{.}7686$, $p=1{.}27\times10^{-6}$, skewness $=2{.}200$). For $\Delta$PSNR, neither model's values showed this kind of unevenness (Single Model: $W=0{.}9742$, $p=0{.}4675$; Dual Model: $W=0{.}9734$, $p=0{.}4427$), which supported using the $t$-test for the average improvement in both models.
...
Because each doctor judges many cases, and each case is judged by many doctors, the 95\% confidence intervals were obtained with a two-way cluster bootstrap that resampled readers and cases independently. This approach avoids treating the 75 responses as fully independent. The observed preference favored the Single Model in 42/75 presentations (56.0\%), whereas the artifact input was preferred in 21/75 (28.0\%). The remaining 12/75 presentations (16.0\%) were judged equivalent.
