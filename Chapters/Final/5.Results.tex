...
\midrule
Total Readers & 6 \\
Total valid responses (readers $\times$ 15 main cases) & 90 \\
Data collection start date & 27-08-2026 \\
Data collection closing date & 13-09-2026 \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
...
\midrule
Jurisdiction & Brazil & 6 & 100 \\
\midrule
\multirow{2}{*}{CT usage frequency} & Monthly & 4 & 66.7 \\
                                          & Daily & 2 & 33.3 \\
\midrule
\multirow{4}{*}{CT experience (years)} & $<2$ & 2 & 33.3 \\
                                            & 2--5 & 2 & 33.3 \\
                                            & 6--10 & 1 & 16.7 \\
                                            & $>10$ & 1 & 16.7 \\
\midrule
\multirow{3}{*}{Experience with artifact reduction} & None & 4 & 66.7 \\
                                                         & Occasional & 1 & 16.7 \\
                                                         & Frequent & 1 & 16.7 \\
\midrule
\multirow{2}{*}{Monitor type} & Conventional & 4 & 66.7 \\
                                  & Unknown & 2 & 33.3 \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
...
\midrule
Radiology and Diagnostic Imaging & 2 & 33.3 \\
Internal Medicine or General Medicine & 1 & 16.7 \\
Orthopedics or Traumatology & 1 & 16.7 \\
Infectious Diseases & 1 & 16.7 \\
Other medical specialty & 1 & 16.7 \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
...
\midrule
Preferred output (Single Model) & 50 & 55.6 & [30.0; 78.9] \\
Preferred input (with artifact) & 23 & 25.6 & [10.0; 44.4] \\
No difference & 17 & 18.9 & [4.4; 38.9] \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
\caption[Primary outcome: anatomical preservation preference]{Paired preference regarding preservation of the anatomical region underlying the metal artifact.\tablecaptionnote{Pooled frequencies over 90 valid responses (6 readers $\times$ 15 main cases). 95\% CIs are percentile intervals from a two-way cluster bootstrap over readers and cases (10,000 resamples; seed 2024).}}
...
\midrule
Output preference $-$ input preference & $+30.0$ pp & [$-11.1$; $+66.7$] pp \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
...
\textbf{Response} & \textbf{N} & \textbf{\%} & \textbf{95\% CI} \\
\midrule
Preferred output (lower artifact intensity) & 73 & 81.1 & [64.4; 94.4] \\
Preferred input & 9 & 10.0 & [1.1; 21.1] \\
No difference & 8 & 8.9 & [0.0; 23.3] \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
\caption[Secondary outcome: artifact intensity]{Paired preference regarding the perceived intensity of metal artifacts.\tablecaptionnote{$n=90$ responses. 95\% CIs by two-way cluster bootstrap over readers and cases (10,000 resamples; seed 2024).}}
...
\textbf{Response} & \textbf{N} & \textbf{\%} & \textbf{95\% CI} \\
\midrule
Preferred output for clinical interpretation & 49 & 54.4 & [30.0; 77.8] \\
Preferred input & 27 & 30.0 & [13.3; 50.0] \\
No difference & 14 & 15.6 & [3.3; 33.3] \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
\caption[Secondary outcome: clinical preference]{Paired preference regarding the image chosen for clinical interpretation.\tablecaptionnote{$n=90$ responses. 95\% CIs by two-way cluster bootstrap over readers and cases (10,000 resamples; seed 2024).}}
...
Uncertain & 1 & 16.7 \\
Probably yes & 3 & 50.0 \\
Definitely yes & 1 & 16.7 \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
\caption[Secondary outcome: complementary utility]{Perceived complementary usefulness of a processed version relative to the original\acrshort{ct} exam.\tablecaptionnote{One response per reader (not per case). $n=6$ completed readers.}}
\label{tab:external-secondary-utility}
\end{table}
\FloatBarrier

Intra-reader consistency was assessed through two hidden repeats per reader: the same case is re-presented later in the session, without signaling, in the same left/right position as the first presentation. Because the responses are nominal and only 12 original--repeat pairs are available, the result is reported as exact agreement for each item (\autoref{tab:external-intra-reader-reliability}).
...
\midrule
Artifact intensity & 7/12 (58.3\%) & 12 \\
Anatomical visualization & 6/12 (50.0\%) & 12 \\
Other-region distortion & 6/12 (50.0\%) & 12 \\
Clinical preference & 7/12 (58.3\%) & 12 \\
\bottomrule
\end{tabular}
\captionsetup{skip=2pt}
\caption[Intra-reader repeat agreement]{Intra-reader exact agreement in the hidden repeats.\tablecaptionnote{Two cases were repeated for each of the six readers, yielding 12 paired original--repeat responses per outcome.}}
...
In this completed panel, the preference frequencies describe how the six readers judged these 15 cases. They do not establish a causal effect on diagnostic or therapeutic outcomes and cannot be generalized beyond this reader--case sample. The absence of a clean reference in these external cases means that the assessment of anatomical accuracy depends entirely on readers' judgment, rather than on an objective error metric. The wide cluster-bootstrap intervals, the small number of readers, and the limited repeat agreement further constrain precision and preclude meaningful subgroup analyses by specialty or experience.
